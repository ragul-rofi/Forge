...
import { supabase, signInStudent, signUpStudent, signInWithGoogle } from '../lib/supabase'
...
export default function Result() {
  ...
  async function handleSaveRoadmap(e) {
    ...
    try {
      ...
      if (data?.user && !data?.session) {
        ...
        setAuthError('')
        setSuccessMessage(
          `Account created! Check your email (${normalizedEmail}) for a confirmation link. ` +
          `After confirming, sign in to access your roadmap.`
        )
        // Store sessionId so they can access results after confirmation
  ...
  async function handleGoogleSignIn() {
    try {
      setAuthError('')
      setSuccessMessage('')
      // Store session ID for post-OAuth redirect
      localStorage.setItem('pendingSessionId', sessionId)
      localStorage.setItem('pendingDomain', domain)
      localStorage.setItem('pendingProfile', session.primary_profile)
              ...
              <form onSubmit={handleSaveRoadmap} className="space-y-3">
                ...
                {successMessage && (
                  <div style={{
                    ...
                    <p className="text-xs" style={{ color: '#22c55e', margin: 0 }}>
                      {successMessage}
                    </p>
                  </div>
                )}
                ...
                <button
                  ...
                  style={{
                    ...
                    opacity: authLoading || successMessage ? 0.6 : 1,
                  }}
                >
